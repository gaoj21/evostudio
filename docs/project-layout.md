...
Python 后端入口为 `backend.api.app:main`，也可以使用 `uv run uvicorn backend.api.app:app --reload`。默认前端构建目录为 `frontend/dist`，默认平台运行数据为 `backend/data`；`EAX_STUDIO_DATA_DIR` 仍可覆盖运行数据位置。
