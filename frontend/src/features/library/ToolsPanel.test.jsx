...
describe('a toolkit can be uploaded as a folder', () => {
  ...
  it('checks loose .py files before keeping anything', async () => {
    api.uploadCustomTool.mockResolvedValue({ name: 'mylib', kind: 'class', tool_class: 'Client',
      configuration: [{ name: 'token', type: 'str' }],
      tools: [{ name: 'fetch', description: 'Fetch it.', params: [] }],
      package: { entry: 'mylib/__init__.py', files: 2, requirements: [] } });
    render(<ToolsPanel onAdd={vi.fn()} />);
    const user = userEvent.setup();
    await user.click(await screen.findByRole('button', { name: /upload folder/i }));

    const files = [new File(['x'], 'tools.py', { type: 'text/x-python' }),
                   new File(['y'], 'client.py', { type: 'text/x-python' })];
    await user.upload(screen.getByLabelText(/or \.py files/i), files);
    await user.click(screen.getByRole('button', { name: 'Check' }));

    await waitFor(() => expect(api.uploadCustomTool).toHaveBeenCalledWith(
      files, { name: '', entry: '', check: true }));
    expect(await screen.findByLabelText('Discovered tools')).toHaveTextContent('fetch');
    expect(screen.getByRole('status')).toHaveTextContent('Would save');
    expect(screen.getByRole('status')).toHaveTextContent('Edit code and configuration');
  });
