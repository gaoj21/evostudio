import { describeBatch as batchState, describeRun, toneClass } from './runStates.js';
import { resumeLabel } from './batchControl.js';
import React, { useEffect, useState } from 'react';
import ResourceActions from '../../components/ResourceActions.jsx';
import { api } from '../../api.js';
import BatchCompare from '../evaluation/BatchCompare.jsx';
...
function statusClass(status, counts) {
  const tone = counts === undefined ? describeRun(status).tone : batchState(status, counts).tone;
  return toneClass(tone).replace('status-', '');
}
...
function RunRows({ runs, onOpenRun }) {
  return (runs || []).map((run) => (
    <ResourceActions key={run.run_id} name={`Run ${run.run_id}`} items={[{ label: 'Open run', action: () => onOpenRun(run) }]}><button
      key={run.run_id}
      type="button"
      className="run-row"
    ...
    >
      <span className={`node-status status-${statusClass(run.status)}`}>{describeRun(run.status).label}</span>
...
function BatchRows({ batches, onOpenBatch, onResumeBatch, picked, onPick }) {
  return (batches || []).map((batch) => (
    <ResourceActions key={batch.batch_id} name={`Batch ${batch.batch_id}`} items={[{ label: 'Open batch', action: () => onOpenBatch(batch) }, ...(onResumeBatch && ['cancelled', 'failed', 'interrupted'].includes(batch.status) ? [{ label: resumeLabel(batch), action: () => onResumeBatch(batch) }] : [])]}><div className="run-row-wrap">
      <input
        type="checkbox"
    ...
    >
      <span className={`node-status status-${statusClass(batch.status, batch.counts || {})}`}>
        {batchState(batch.status, batch.counts || {}).label}
      </span>
      <span className="run-row-main">
      ...
      {batch.evaluation && (
        <span className="batch-row-eval muted small" title="Evaluation">{batch.evaluation}</span>
      )}
      ...
      {/* Beside the row, not inside it: a button in a button is not a thing. */}
      {onResumeBatch && resumeLabel(batch) && (
        <button
          type="button"
...
export default function RunsPanel({ open, graphId, onClose, onOpenRun, onOpenBatch, onResumeBatch }) {
          ...
          <>
            ...
            <div className="runs-list">
              {tab === 'runs'
                ...
                : <BatchRows batches={batches} onOpenBatch={onOpenBatch} onResumeBatch={onResumeBatch}
