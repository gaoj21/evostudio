...
describe('the batch preview is authoritative', () => {
  ...
  it('says what stopping a batch does and what it cannot undo', async () => {
    // Stop interrupts the records in flight; what it cannot take back is the
    // requests already sent to the provider, and that is what to say.
    const user = open();
    await batchTab(user);
    const note = await screen.findByText(/Stopping a batch/i);
    expect(note).toHaveTextContent(/interrupts the ones running/i);
    expect(note).toHaveTextContent(/still paid for/i);
    expect(note).not.toHaveTextContent(/already running will finish/i);
  });
});
