...
describe('defaultFor', () => {
  it('gives each type a starting value the field can render', () => {
    // A boolean field is a three-way select now; its values are strings.
    expect(defaultFor('bool')).toBe('false');
    expect(defaultFor('bool', false)).toBe('');   // optional: not provided
...
describe('toField', () => {
  ...
  it('leaves text and booleans alone', () => {
    ...
    expect(toField('bool', undefined)).toBe('');
  });
