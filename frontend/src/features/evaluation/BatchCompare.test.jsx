...
describe('BatchCompare', () => {
  ...
  it('shows what the changed record produced', () => {
    show();
    // Shown as a tree now: the key and the value are separate elements.
    const trees = screen.getAllByTestId('json-view');
    expect(trees[0].textContent).toContain('answer');
    expect(trees[0].textContent).toContain('"?"');
  });
