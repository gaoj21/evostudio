...
export default function EvaluatorInspector({node,onUpdate,onRename,getGraph}) {
 ...
 // Only new code can change which metrics exist: a timing, time limit or
 // parameter edit must not take the detected metrics — and the objective
 // picker with them — away.
 const update=patch=>{version.current+=1;setReport(null);if('code' in patch)setSchema(null);onUpdate(node.id,{evaluator:{...cfg,...patch,...('code' in patch?{metric:'',output_schema:null}:{})}});};
 const preview=async()=>{
 ...
 <div className="field"><label htmlFor="eval-metric">Evolve objective</label><select id="eval-metric" value={cfg.metric || ''} onChange={e=>onUpdate(node.id,{evaluator:{...cfg,metric:e.target.value}})}>
 {/* An objective the metrics no longer contain is not silently shown as the first one. */}
 {!(report?.metrics || cfg.output_schema).some(f=>f.name===cfg.metric) && <option value="">Choose the metric Evolve optimizes</option>}
 {(report?.metrics || cfg.output_schema).map(f=><option key={f.name} value={f.name}>{f.name}</option>)}</select></div></> : <p className="muted small">Preview to identify metric names, values and details before using this evaluator in Evolve.</p>}
