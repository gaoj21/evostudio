...
export default function EvaluatorInspector({node,onUpdate,onRename,getGraph}) {
 ...
 {python && <div className="field"><label htmlFor="eval-timeout">Time limit (seconds)</label><input id="eval-timeout" type="number" min={10} max={3600} step={1} value={cfg.timeout ?? 120} onChange={e=>update({timeout:Number(e.target.value)})}/><small className="muted">How long your code may run on one evaluation before it is stopped (10–3600).</small></div>}
