...
describe('the detected metrics and the objective', () => {
  const metrics = [{ name: 'accuracy', type: 'number', sample: 1 }];

  it('keeps the detected metrics when only the timing changes', async () => {
    const update = vi.fn();
    const cfg = { type: 'python', code: 'def evaluate(records):\n    return {"metrics": {"accuracy": 1}}', metric: 'accuracy', output_schema: metrics };
    const { container } = render(<EvaluatorInspector node={{ id: 'check', data: { evaluator: cfg } }} onUpdate={update} onRename={vi.fn()} getGraph={() => ({ id: 'g', tasks: [], edges: [] })} />);
    await userEvent.selectOptions(within(container).getByLabelText('Run evaluation'), 'batch');
    expect(update).toHaveBeenCalledWith('check', { evaluator: expect.objectContaining({ timing: 'batch', metric: 'accuracy', output_schema: metrics }) });
  });

  it('asks for a choice rather than showing an objective that is not in the metrics', () => {
    const cfg = { type: 'python', code: 'x', metric: 'renamed_away', output_schema: metrics };
    const { container } = render(<EvaluatorInspector node={{ id: 'check', data: { evaluator: cfg } }} onUpdate={vi.fn()} onRename={vi.fn()} getGraph={() => ({ id: 'g', tasks: [], edges: [] })} />);
    expect(within(container).getByLabelText('Evolve objective')).toHaveValue('');
  });
});
