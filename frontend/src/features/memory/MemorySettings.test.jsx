  ...
  it('drops a field from the policy when it is unticked', async () => {
    ...
    expect(onUpdate).toHaveBeenCalledWith('judge', {
      memory: { outputs: null, inputs: ['company'], when: 'success', retrieve: 3,
                ...
                read_from: null, read: null, read_enabled: true, write_enabled: true },
    });
  });
...
describe('recording a key inside a JSON answer', () => {
  // `detection.source` is a key inside detect's answer, not a field of the
  // run, so the tick-list cannot offer it; it is typed and checked.
  it('adds a dotted key to the context once it names a real field', async () => {
    const { onUpdate, user } = setup({}, [
      ...SIBLINGS, { name: 'detect', fields: ['detection'], outputs: ['detection'] }]);
    const box = screen.getByLabelText('Record a key inside a field');
    await user.type(box, 'detection.source{Enter}');

    expect(onUpdate).toHaveBeenCalledWith('judge',
      { memory: expect.objectContaining({ context: ['detection.source'] }) });
  });

  it('refuses a key on a field the run does not have', async () => {
    const { onUpdate, user } = setup();
    await user.type(screen.getByLabelText('Record a key inside a field'), 'nothing.source');
    expect(screen.getByRole('button', { name: 'Record key' })).toBeDisabled();
    expect(screen.getByText(/No field named nothing/)).toBeInTheDocument();
    expect(onUpdate).not.toHaveBeenCalled();
  });

  it('shows a recorded key and lets it be removed', async () => {
    const { onUpdate, user } = setup({ memory: { context: ['detection.source'] } });
    // The key also appears as a 'Dated by' option; the recorded row is the
    // one that says where the key lives.
    const row = screen.getByText(/key inside detection/).closest('label');
    await user.click(row.querySelector('input'));
    expect(onUpdate).toHaveBeenCalledWith('judge',
      { memory: expect.objectContaining({ context: [] }) });
  });
});

it('preserves custom storage type and budget when changing read policy', async () => {
  const { user, onUpdate } = setup({ memory: { kind: 'recall', limit: 9000, match: 'company', extension: 'keep' } });
  await user.click(screen.getByRole('checkbox', { name: 'Read memory before execution' }));
  expect(onUpdate).toHaveBeenCalledWith('judge', { memory: expect.objectContaining({
    kind: 'recall', limit: 9000, extension: 'keep', read_enabled: false,
  }) });
});

it('allows read-only memory without a write-field warning', () => {
