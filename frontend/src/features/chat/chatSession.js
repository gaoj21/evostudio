...
export function useChatHistory(key) {
  ...
  // A turn that settles after the user moved on still lands in the session it
  // was asked in, not whichever one is open now.
  const updateSession = useCallback((sessionId, update) => change(current => {
    const session = current.sessions.find(s => s.id === sessionId);
    if (!session) return current;
    return updateMessages(current, sessionId, typeof update === 'function' ? update(session.messages) : update);
  }), [change]);
  const latest = useRef(data);
  latest.current = data;
  const messagesOf = useCallback(sessionId => latest.current.sessions.find(s => s.id === sessionId)?.messages || [], []);
  ...
  return [messages, setMessages, { context: data.sessions.find(s => s.id === activeId)?.context, updateContext, sessions: data.sessions, activeId, create, select, rename, remove, updateSession, messagesOf }];
}
