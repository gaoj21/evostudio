...
export default function ChatPanel({
  ...
  const submit = useCallback(async (text, { silent = false } = {}) => {
    ...
    setBusy(true);
    try {
      const res = await api.chatGraph(graphId, { message: text, history, graph: before, request_id: turn.id }, { signal: turn.controller.signal });
      if (activeGraph.current !== graphId || turn.stopped || !cancellation.isCurrent(turn)) return;
      const id = newId();
      const nextMessages = [
        ...messagesRef.current,
        {
          ...
          snapshot: res.applied ? { ...before, id: res.graph?.id || before.id } : null,
  ...
  }, [graphId, getGraph, onApply, onSaved, onOpenPanel, pollJob, setMessages, cancellation.begin, cancellation.isCurrent, cancellation.finish]);
