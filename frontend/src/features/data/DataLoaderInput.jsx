...
export default function DataLoaderInput({ config, onChange, getGraph, nodeId }) {
  ...
  // The sample request in flight, so an edit, Cancel or leaving the node
  // ends its Dataset worker instead of leaving it running to its time limit.
  const sampling = useRef(null);
  const [samplingId, setSamplingId] = useState(null);
  const stopSampling = () => {
    const id = sampling.current;
    sampling.current = null; setSamplingId(null);
    if (id) api.stopDataLoaderPreview?.(id).catch(() => {});
  };
  useEffect(() => () => { const id = sampling.current; sampling.current = null; if (id) api.stopDataLoaderPreview?.(id).catch(() => {}); }, []);
  const update = patch => {
    stopSampling();
  ...
  async function sampleOutputs() {
    ...
    const requestId = `sample-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`;
    sampling.current = requestId; setSamplingId(requestId);
    setBusy(true); setError('');
    try {
      const result = await api.previewDataLoader({...config,preview_mode:'sample',_request_id:requestId,...(getGraph ? {_graph:getGraph(),_node:nodeId} : {})});
      ...
      if (sampling.current === requestId) { sampling.current = null; setSamplingId(null); }
      setBusy(false);
    }
  }
  ...
  return <section aria-label="DataLoader">
    ...
    {!legacy && <>
      <div className="input-actions"><button type="button" className="primary" disabled={busy || codeDirty || !inputSchema || !config.resource_id} onClick={sampleOutputs}>{samplingId ? 'Sampling…' : 'Sample 1 record (detect outputs)'}</button>{samplingId && <button type="button" onClick={stopSampling}>Cancel sampling</button>}</div>
    ...
    {error && <p role="alert" style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{String(error)}</p>}
