...
vi.mock('../../api.js',()=>({api:{listDataResources:vi.fn(),listCustomTools:vi.fn(),uploadDataResource:vi.fn(),previewDataLoader:vi.fn(),stopDataLoaderPreview:vi.fn(),deleteDataResource:vi.fn(),attachDataResource:vi.fn(),listSourceTypes:vi.fn(),inspectDataLoaderCode:vi.fn()}}));
...
describe('a slow sample',()=>{
 it('can be cancelled, and editing the Input stops the Dataset worker',async()=>{
  let settle;
  api.previewDataLoader.mockImplementation(()=>new Promise(resolve=>{settle=resolve;}));
  api.stopDataLoaderPreview.mockResolvedValue({status:'stopping'});
  render(<DataLoaderInput config={{loader:'python',resource_id:'data',code:'def build_dataset(resource): pass'}} onChange={vi.fn()}/>);
  const button=await waitFor(()=>{const b=screen.getByRole('button',{name:'Sample 1 record (detect outputs)'});expect(b).toBeEnabled();return b;});
  await userEvent.click(button);
  const cancel=await screen.findByRole('button',{name:'Cancel sampling'});
  await userEvent.click(cancel);
  const [requestId]=api.stopDataLoaderPreview.mock.calls[0];
  expect(api.previewDataLoader).toHaveBeenCalledWith(expect.objectContaining({_request_id:requestId}));
  settle({fields:[{name:'i',type:'int'}],preview:[],preview_mode:'sample',sample_limit:1});
  await waitFor(()=>expect(screen.queryByRole('button',{name:'Cancel sampling'})).toBeNull());
 });
 it('stops the in-flight sample when a setting changes',async()=>{
  api.previewDataLoader.mockImplementation(()=>new Promise(()=>{}));
  api.stopDataLoaderPreview.mockResolvedValue({status:'stopping'});
  render(<DataLoaderInput config={{loader:'python',resource_id:'data',code:'def build_dataset(resource): pass'}} onChange={vi.fn()}/>);
  await waitFor(()=>expect(screen.getByRole('button',{name:'Sample 1 record (detect outputs)'})).toBeEnabled());
  await userEvent.click(screen.getByRole('button',{name:'Sample 1 record (detect outputs)'}));
  await screen.findByRole('button',{name:'Cancel sampling'});
  await userEvent.selectOptions(screen.getByLabelText('How to use the data'),'reference');
  await waitFor(()=>expect(api.stopDataLoaderPreview).toHaveBeenCalled());
 });
});
