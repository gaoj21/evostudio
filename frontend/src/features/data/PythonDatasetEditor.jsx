...
export default function PythonDatasetEditor({code, onChange, disabled, kind='Dataset', example=DATASET_EXAMPLE}) {
  ...
  return <div className="field" onDragOver={e=>{e.preventDefault();e.stopPropagation();}} onDrop={e=>{e.preventDefault();e.stopPropagation();if(!disabled)load(e.dataTransfer.files[0]);}}>
    ...
    <textarea id="loader-python-code" spellCheck={false} rows={18} style={{fontFamily:'monospace',minHeight:260,resize:'vertical'}} value={code || ''} disabled={disabled} onChange={e=>onChange(e.target.value)} placeholder={kind==='Dataset'?"from torch.utils.data import Dataset…":"def evaluate(records):…"}/>
    <input ref={input} aria-label={`Upload Python ${kind}`} type="file" accept=".py" hidden onChange={e=>{load(e.target.files[0]);e.target.value='';}}/>
    <div className="input-actions"><button type="button" disabled={disabled} onClick={()=>input.current.click()}>Upload .py</button><button type="button" disabled={disabled} onClick={()=>{if(!code || window.confirm('Replace the current code with the example?'))onChange(example);}}>Load example</button></div>
