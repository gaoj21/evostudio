...
export default function PythonDatasetEditor({code, onChange, disabled, kind='Dataset', example=DATASET_EXAMPLE, examples, onDirtyChange}) {
  ...
  return <div className="field" onDragOver={e=>{e.preventDefault();e.stopPropagation();}} onDrop={e=>{e.preventDefault();e.stopPropagation();if(!disabled)load(e.dataTransfer.files[0]);}}>
    ...
    {kind==='Dataset' ? <p className="muted small">Multiple classes and helper functions are allowed. Studio calls only the single top-level build_dataset factory. Paste code or drop a .py file here. Define build_dataset(resource, path: str, split: str = "dev") returning a PyTorch Dataset or IterableDataset. Typed parameters generate the input form; legacy config parameters are also supported. Each item must be a JSON-compatible dictionary. Studio creates the DataLoader and keeps the final partial batch. For large datasets, declare OUTPUT_SCHEMA to inspect fields without executing code. Only the explicit Sample action reads up to 5 items; your Dataset constructor must avoid loading the whole file.</p> : kind==='Tool' ? <p className="muted small">Define build_tool with typed configuration parameters, returning an object with run(inputs). Declare INPUT_SCHEMA and OUTPUT_SCHEMA at module level. Multiple internal helpers/classes are allowed. Existing documented function tools are also supported.</p> : <p className="muted small">Multiple classes and helper functions are allowed. Define one top-level build_evaluator with typed parameters, returning an object with evaluate(records). Only factory parameters generate the form. The method returns metrics with optional records, coverage and details. Legacy top-level evaluate(records, ...) is supported; do not define both entrypoints.</p>}
    ...
    {kind==='Dataset' ? <p className="muted small">resource.root is the uploaded folder; resource.files lists relative file paths. Reader config is passed as config. Code is saved with this Input; Apply OUTPUT_SCHEMA reads declared fields without execution. If your code has no declaration, use Sample 5 records to detect and apply fields. Sample explicitly executes the Dataset. Use installed Python packages.</p> : kind==='Tool' ? <p className="muted small">Confirming code only reads the interface. Test inputs and Run example execute it. Configuration is shared by every caller of this saved tool; save under another name for different settings.</p> : <p className="muted small">records contains complete saved inputs, node outputs, final results and errors, including unconnected nodes. Your code owns grouping and scoring. Code is saved in this node; installed Python packages are available.</p>}
