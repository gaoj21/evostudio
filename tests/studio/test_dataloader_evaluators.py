...
def test_pytorch_code_preview_and_batch_use_same_records_and_keep_tail(client):
    ...
    # The sample is one record; a full read starts with the same record.
    assert [{k:v for k,v in row.items() if k != '_dataloader'} for row in dataloaders.records(config)][:1] == response.json()['preview']
