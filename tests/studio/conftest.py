...
paths point at the developer's real `backend/data/`, and a test that creates a
...
# one defaults to the developer's real `backend/data/`.
_DATA_PATHS = [
...
def _never_touch_real_data(tmp_path, monkeypatch):
    ...
    pointing at the real `backend/data/`: a run started by a test wrote its
