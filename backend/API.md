...
Mutable state is stored under `backend/data/` by default for backward
...
`backend/data/memory/<graph_id>/<task_name>/` (SQLite + FAISS, corpus id
...
User-defined tools live at `backend/data/tools/<name>.json`:
...
is code the workflow *calls*. Stored as `backend/data/skills/<name>/SKILL.md`
...
Every run writes complete artifacts under `backend/data/workspace/<graph_id>/`:
...
- `POST /v1/chat/completions` — the usual body. Honoured fields:
  ...
  - **`user`** keys the agent's long-term memory, so a client that sets it gets
    ...
    `backend/data/agent-memory/<user>/` and are isolated per user.
...
- Runs execute in a daemon thread; state kept in a module-level dict and
  persisted to `backend/data/runs/{run_id}.json` on completion.
...
- Batches persist to `backend/data/batches/{batch_id}.json` on completion;
