...
def _workflow_py(goal, llm_tasks, tool_nodes, edges, skills, project, local_modules,
    ...
    edge_pairs = [{"source": e.get("source"), "target": e.get("target"),
                   **({"control_only": True} if e.get("control_only") else {}),
                   **({"mappings": e.get("mappings")} if e.get("mappings") else {})}
                  for e in edges]
    # The order and the data flow, exactly as Studio compiled them. Source
    # nodes are not executed by an export — their fields are the inputs the
    # caller supplies — so a binding from one is read from the inputs.
    source_names = {n["name"] for n in (plan_nodes or []) if n.get("kind") == "source"}
    steps = [{**n, "input_bindings": {
                  k: ({"external": v["field"]} if v.get("node") in source_names else v)
                  for k, v in (n.get("input_bindings") or {}).items()}}
             for n in (plan_nodes or []) if n.get("kind") != "source"]
...
# Deterministic nodes: plain function calls, no LLM. Where they run is
# decided by PLAN, like every other node.
TOOL_NODES = {_py(tool_specs)}

EDGES = {_py(edge_pairs)}

# The execution plan Studio compiled from the canvas: every node in order,
# with what feeds each of its inputs. `run()` walks this and nothing else —
# no dependency is inferred from field names that happen to match.
PLAN = {_py(steps)}
...
_MODULES = {{}}
# Class toolkits' instances, built once per process.
_INSTANCES = {{}}
...
def load_custom_module(name, spec):
    ...
    with _in_folder(folder):
        if folder is not None:
            # As in Studio: an entry inside a package keeps its dotted name,
            # so its relative imports work.
            import tool_runtime

            ns, _ = tool_runtime.load_entry(code, folder, package.get("entry") or "tools.py",
                                            module_name=f"studio_tool_{{name}}")
        else:
...
def custom_callable(name, spec, tool):
    ...
    if spec.get("kind") == "class":
        # A class toolkit: one instance, built from the saved configuration
        # on first use and kept, and each tool is one of its methods.
        import tool_runtime

        def call(**kwargs):
            with _in_folder(folder):
                if name not in _INSTANCES:
                    _INSTANCES[name] = tool_runtime.build_instance(
                        ns, spec.get("target"), spec.get("configuration") or [],
                        spec.get("config") or {{}})
                return tool_runtime.call_method(_INSTANCES[name], tool["name"], kwargs)
        return call
...
def call_tool_node(node, args):
    """Run one deterministic node and spread its result over its outputs."""
    ...
    if not isinstance(result, dict):
        raise SystemExit(f"Tool node '{{node['name']}}' declares outputs {{names}} "
                         f"but returned a {{type(result).__name__}}")
    absent = [n for n in names if n not in result]
    if absent:
        raise SystemExit(f"Tool node '{{node['name']}}' returned no {{absent}}")
...
def prepare_ltm(tasks):
    ...
    for task in tasks:
        ...
        if not memory_policy.policy(task)["write_enabled"]:
            continue
    ...
    for task in tasks:
        if not task.get("use_long_term_memory") or not memory_policy.policy(task)["read_enabled"]:
            continue
        for other in (memory_policy.stores_read_by(task) if memory_policy.stores_read_by(task) is not None else [task.get("name")]):
            source_task = next((t for t in tasks if t.get("name") == other), {{}})
...
class _Env:
    """What memory reads: everything the run produced, as one flat dict."""
...
def _gather(spec, bindings, values, external):
    """A node's inputs: from the edges that feed it, else from the inputs given."""
    found, missing = {{}}, []
    for p in spec.get("inputs") or []:
        name = p["name"]
        b = bindings.get(name)
        if b and "external" in b and b["external"] in external:
            found[name] = external[b["external"]]
        elif b and b.get("node") in values and b.get("field") in values[b["node"]]:
            found[name] = values[b["node"]][b["field"]]
        elif name in external:
            found[name] = external[name]
        elif p.get("required", True):
            missing.append(name)
    if missing:
        raise SystemExit(f"Node '{{spec['name']}}' is missing inputs: {{missing}}")
    return found


async def _run_llm(agent, task, inputs):
    from evoagentx.core.message import MessageType
    message = await agent.async_execute(
        action_name=agent.customize_action_name, action_input_data=inputs,
        return_msg_type=MessageType.RESPONSE, wf_goal=GOAL,
        wf_task=task["name"], wf_task_desc=task.get("description", ""))
    content = message.content
    data = (content.get_structured_data() if hasattr(content, "get_structured_data")
            else content if isinstance(content, dict) else {{}}) or {{}}
    return {{o["name"]: data[o["name"]] for o in task.get("outputs") or [] if o["name"] in data}}


def run(inputs):
    """Execute the workflow the way Studio compiled it and return its result.

    One node at a time, in PLAN order; each LLM node is its own framework
    graph so the framework builds its agent but decides nothing about order
    or data flow. What crosses an edge is what the edge says.
    """
    import asyncio
    ...
    try:
        ...
        ok = False
        save_ltm(memories, graph, env, succeeded=False)
        raise
    save_ltm(memories, graph, env, succeeded=ok)
    result = {{}}
    for node in PLAN:
