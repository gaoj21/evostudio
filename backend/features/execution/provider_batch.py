...
def attach_workflow_model(model, state):
    ...
    from .token_usage import record, response_usage
    def counted(value):
        # The transport is replaced, so the model's own usage hook never sees
        # these responses: count what each result reports as it arrives.
        if not isinstance(value, str):
            record(state, response_usage(value))
        return _text(value)
