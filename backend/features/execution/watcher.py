...
def _watch_loop(graph: dict, node: dict, stop: threading.Event, state: dict) -> None:
    ...
    while not stop.is_set():
        ...
        try:
            ...
            if stop.is_set():
                # Stop landed while this poll was in flight (a fetch with its
                # own retries and pauses takes a while). It used to return and
                # then start a run per record — Stop had already reported the
                # watcher gone. The records stay unseen, so a later start runs
                # them rather than skipping them as already handled.
                break
            for record in records:
