    ...
    if loader == 'python':
        ...
        try:
            if sample_limit is None:
                # A full read streams batch by batch from one worker: each
                # batch gets the time allowed per batch however long the
                # dataset, and Stop ends the worker between batches.
                from .dataset_stream import read
                control = chat_control.current.get()
                rows = [row for chunk in read(config, (lambda: control.event.is_set()) if control else (lambda: False))
                        for row in chunk]
                check_cancel()
            ...
            rows = chat_control.worker('dataset', {'code':config['code'],
                ...
                timeout=preview_timeout(config))
        except SourceError:
            raise
        except Exception as exc:
...
def _prepare_cached(config):
    ...
    for entry in resource['files']:
        ...
        if _unchanged(path, entry['sha256']):
            continue
    ...
    with _cache_lock_for(key):
        ...
        _prune_cache(root, path)
        return rows, meta


# Every edit of the code or the configuration prepares (and stores) the whole
# dataset again, so the cache is bounded: the least recently used entries go.
CACHE_LIMIT_BYTES = max(0, int(float(os.getenv('EVO_DATALOADER_CACHE_MB', '2048')) * 1024 * 1024))


def _prune_cache(root, keep):
    if not CACHE_LIMIT_BYTES:
        return
    try:
        entries = sorted((path.stat().st_mtime, path.stat().st_size, path) for path in root.glob('*.json'))
    except OSError:
        return
...
# A resource's files never change (a new upload is a new resource), so the
# same bytes are not hashed again on every preview, run and Evolve round.
_verified: dict = {}


def _unchanged(path, expected):
    """The file still has the digest the manifest recorded."""
    try:
        state = path.stat()
    except OSError:
        return False
...
# Sample requests are identified by the caller so they can be stopped: a
# Dataset that loads a large model can take minutes to produce one record.
PREVIEW_SCOPE = 'dataloader-preview'


@router.post('/preview/{request_id}/stop')
def stop_preview(request_id: str):
    from backend.features.chat import chat_control
    try:
        control = chat_control.get_control(PREVIEW_SCOPE, request_id)
    except ValueError as exc:
        raise HTTPException(422, str(exc)) from exc
    control.event.set()
...
def preview(config: dict = Body(...)):
    from backend.features.chat import chat_control
    request_id = (config or {}).get('_request_id')
    if request_id is None:
        return _preview(config)
    try:
        control = chat_control.get_control(PREVIEW_SCOPE, request_id, start=True)
    except ValueError as exc:
        raise HTTPException(422, str(exc)) from exc
    ...
    try:
        ...
        raise HTTPException(409, 'Sampling stopped; the Dataset worker was ended.') from None
    finally:
