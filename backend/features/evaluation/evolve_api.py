...
def list_evolve_tasks(graph_id: str | None = None):
    ...
    return [
        {k: t.get(k) for k in (
            ...
            "params", "created_at", "finished_at", "elapsed_seconds", "source",
        )} | {
...
def apply_evolve_result(task_id: str, mode: str = "new"):
    ...
    # Only the prompts move: the workflow as it is now, with the optimized
    # instructions, rather than the snapshot the optimizer ran on.
