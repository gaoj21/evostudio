...
Canvas Chat needs a LangChain chat model that supports tool calling. It gets one from `backend.features.model_bridge.agent_model(provider)`, which wraps the `llm` package; the harness never names a provider. Missing imports are reported with the backend Python path, while arbitrary provider exceptions remain redacted.
