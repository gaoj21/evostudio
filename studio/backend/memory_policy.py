...
def policy(task: dict) -> dict:
    ...
    return {
        ...
        "match": (raw.get("match") or "").strip() or None,
        ...
        "read_from": _names(raw.get("read_from")),
...
def validate(task: dict, siblings: dict | None = None) -> list[str]:
    ...
    if siblings is not None:
        ...
        unknown = [c for c in wanted_context if c not in produced]
        if unknown:
            errors.append(
    ...
    if settings["outputs"] == [] and settings["inputs"] == []:
        errors.append(
...
def with_context(task: dict, inputs: dict, run_data: dict | None) -> dict:
    ...
    for name in policy(task)["context"]:
        if name not in resolved and name in (run_data or {}):
            resolved[name] = (run_data or {})[name]
    return resolved
...
def table_block(rows: list[dict], task: dict, wanted: str, cutoff: str) -> str:
    ...
    lines = []
    for row in rows or []:
        text = _render_payload(row.get("payload") or {}, keep=settings["read"])
        if not text:
            continue
...
async def recall_async(stores, task: dict, inputs: dict, history=None,
    ...
    if settings["kind"] == "table":
        if table is None:
            return ""
        for name in (settings["read_from"] or [task.get("name")]):
