...
export function summarise(policy, outputs, inputs) {
  ...
  const where = policy.match ? `a table per ${policy.match}` : 'a searchable corpus';
 ...
 */
export default function MemorySettings({ node, onUpdate, siblings = [] }) {
  const d = node.data;
  ...
  const patch = (fields) =>
    onUpdate(node.id, { memory: { ...policy, ...fields } });

  const keepsNothing = policy.outputs?.length === 0 && policy.inputs?.length === 0;
    ...
    <div className="field">
      ...
      {!enabled ? (
        ...
        <div className="memory-settings">
          <div className="muted small">{summarise(policy, outputs, inputs)}</div>
          ...
          <div className="memory-row">
            ...
            <span className="muted small">
              similar past runs, whenever they happened
