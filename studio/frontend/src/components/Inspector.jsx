    ...
    <aside className="inspector">
      ...
      {(schema?.config || []).map((f) => (
        <div className="field" key={f.name}>
          ...
          {f.type === 'select' ? (
            <select value={cfg[f.name] ?? f.default ?? ''} onChange={(e) => setCfg({ [f.name]: e.target.value })}>
              {(f.options || []).map((o) => (
...
export default function Inspector({ node, runInfo, runMode, onUpdate, onRename, graph, onGraphChange, producedNames, siblings }) {
    ...
    return (
      <aside className="inspector">
        ...
        {runInfo?.status === 'completed' && runInfo.output != null && (
          <div className="field">
            <label>Output</label>
            <pre className="json-view">{JSON.stringify(runInfo.output, null, 2)}</pre>
          </div>
        )}
      </aside>
    );
    ...
    return (
      <aside className="inspector">
        ...
        <div className="field">
          ...
          <div className="muted small">Deterministic execution, no LLM. Runs before LLM nodes; inputs can come from sources, other tool nodes, or workflow inputs.</div>
        </div>
  ...
  return (
    <aside className="inspector">
      ...
      <MemorySettings node={node} onUpdate={onUpdate} siblings={siblings} />
