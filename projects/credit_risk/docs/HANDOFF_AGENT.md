...
> 最终版,对应代码 `projects/credit_risk/agentic_pipeline/`(934 行,8 个模块)。
> 框架层依赖:`evoagentx/`(LTM、Skill、HITL、Message)。
> 应用层共享件:`projects/credit_risk/credit_risk_demo.py`(记忆助手函数)。
...
- prompt 优化:把 pipeline 包成 MiproRegistry program 喂给
  `MiproOptimizer`,方法照抄 `projects/credit_risk/optimize_mipro.py`(旧版 3 节点
...
运行:`.venv/bin/python projects/credit_risk/agentic_pipeline/run_demo.py`(约 $0.01)
...
`projects/credit_risk/output/store*`(`store_dir` 参数控制)。
...
- ~~`run_contemporary_eval.py`~~ 已完成(dev+test 13 样本:检出 7/8、
  ...
  `projects/credit_risk/output/eval_contemporary/REPORT.md`)。后续:开
