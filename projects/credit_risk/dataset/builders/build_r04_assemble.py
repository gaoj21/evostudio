...
Run from the repository root:
    .venv/bin/python projects/credit_risk/dataset/builders/build_r04_assemble.py
"""
...
BASE = "projects/credit_risk/dataset/contemporary"
...
def assign_splits(samples):
    """dev/test per class, chronological by window.end, DEV_SHARE to dev."""
    for cls in ("positive", "negative"):
        group = sorted([s for s in samples if s["type"] == cls],
                       key=lambda s: s["window"]["end"])
        n_dev = int(round(len(group) * DEV_SHARE))
        for i, s in enumerate(group):
            s["split"] = "dev" if i < n_dev else "test"
...
def main():
    ...
    # Two splits, per class, chronological by window.end: the earlier
    # DEV_SHARE of each class is dev (where prompts are tuned and the
    # optimizer learns), the rest is test (touched only for the final
    # number). No train split: with 41 companies a third way is too thin
    # to mean anything, and the optimizer does its own teach/judge split
    # inside dev. See build_r07_resplit.py for the same rule applied in
    # place.
