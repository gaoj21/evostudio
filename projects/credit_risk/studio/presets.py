"""Credit-risk preset nodes and template graph (loaded by studio_plugin.py).

The four presets mirror projects/credit_risk/agentic_pipeline/agents.py
...
arguments become {company}/{as_of}, sourced from the credit_risk feed or any
source that outputs as_of — the date the evidence runs up to; nothing here
depends on window_start/window_end):
    ...
    return [
        {
            ...
            "defaults": {
                ...
                "inputs": [
                    ...
                    _io("as_of", description="As-of date: evidence runs up to this date (YYYY-MM-DD)"),
                ],
...
REVIEW_RULE = {"node": "decide", "score_field": "score", "range": [35, 65],
...
def credit_risk_template() -> dict:
    ...
    """
    from backend.api.registry import source_presets
    ...
    return {
        ...
        "tasks": tasks,
        "edges": edges,
        # Gray-zone alerts go to a person (Studio's Review panel).
        "review": dict(REVIEW_RULE),
    }
