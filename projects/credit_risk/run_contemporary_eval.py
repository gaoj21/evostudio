...
- negative: only reviewed negatives enter the alert-rate denominator

Checkpointing: one JSON per finished sample under the output dir; re-running
skips them. Safe to interrupt and resume.

Run from the repository root:
    .venv/bin/python projects/credit_risk/run_contemporary_eval.py --split dev
    .venv/bin/python projects/credit_risk/run_contemporary_eval.py --split test
    .venv/bin/python projects/credit_risk/run_contemporary_eval.py --split all --workers 4
"""
...
DATASET_DIR = os.path.join("projects", "credit_risk", "dataset", "contemporary")
OUT_DIR = os.path.join("projects", "credit_risk", "output", "eval_contemporary")
...
def eval_sample(sample: dict, registry, out_dir: str, with_text: bool,
    ...
    rec = {
        ...
        "label": sample["label"],
        # R06 news_quality flags: a flagged sample's stream is not about the
        # company, so a miss there is the data's, and metrics are reported
        # with and without.
        "flags": (sample.get("news_quality") or {}).get("flags") or [],
        "n_items": len(items),
...
def summarize(records: list):
    _summarize(records, "all samples")
    flagged = [r for r in records if r.get("flags")]
    if flagged:
        kept = [r for r in records if not r.get("flags")]
        names = ", ".join(f"{r['company'][:30]} [{','.join(r['flags'])}]" for r in flagged)
        print(f"\nexcluding {len(flagged)} flagged (news_quality): {names}")
        _summarize(kept, "unflagged samples")


def _summarize(records: list, title: str):
    print(f"\n--- {title} ({len(records)}) ---")
    pos = [r for r in records if r["type"] == "positive"]
    candidates = [r for r in records if r["type"] == "negative"]
    neg = [r for r in candidates if (r.get("label") or {}).get("negative_review_status") == "verified"]
    print(f"unverified negatives excluded: {len(candidates)-len(neg)}")
    ...
    print(f"negatives: {len(fa)}/{len(neg)} with alerts (verified-negative alert rate)")
    for r in fa:
